...
for i in range(1,6):
    ...
    with h5py.File(data_dir+"DZNE_CN_FTD_AD.h5", mode='r') as file:
    ...
    for epoch in range(intial_epoch, config.nb_epoch):
        ...
        if valid_loss < best_loss:
            ...
            },os.path.join(config.model_path, f"DZNE_Resnet_{str(model_depth)}.pt"))
            print("Saving model ",os.path.join(config.model_path, f"DZNE_Resnet_{str(model_depth)}.pt"))
    ...
    # weight_dir = os.path.join(config.model_path, f"DZNE_Resnet_{str(model_depth)}.pt")
...
# Writing to CSV
with open(f'metric_dzne_resnet_{str(model_depth)}.csv', mode='w', newline='') as file:
